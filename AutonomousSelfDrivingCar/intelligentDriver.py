...
class IntelligentDriver(Junior):
    ...
    # intialize a global variable to store the previous goal position
    global prevCarPos
    prevCarPos = []
    def getNextGoalPos(self, beliefOfOtherCars: list, parkedCars:list , chkPtsSoFar: int):
        ...
        '''
        goalPos = (0, 0)
        check_points_list = self.checkPoints
        
        (x, y) = check_points_list[chkPtsSoFar]
        goalPos = (util.colToX(x), util.rowToY(y))
                
        numRows, numCols = self.layout.getBeliefRows(), self.layout.getBeliefCols()
        moveForward = True

        currPos = self.getPos() # the current 2D location of the AutoCar (refer util.py to convert it to tile (or grid cell) coordinate)
        currPos = (util.xToCol(currPos.x), util.yToRow(currPos.y))
        
        m = [[0 for _ in range(numCols)] for _ in range(numRows)]
        ...
        for edge in self.worldGraph.edges:
            if edge[0] == currPos:
                # have a high belief at the obstacles
                if m[edge[1][0]][edge[1][1]] > 0.7:
                    pq.put((1000, edge[1]))
                else:
                    pq.put((m[edge[1][0]][edge[1][1]], edge[1]))
                    
        while not pq.empty():
            if pq.queue[0][1] in prevCarPos:
                pq.queue.pop(0)
            else:
                (x, y) = pq.queue[0][1]
                break

        prevCarPos.append((x, y))
        goalPos = (util.colToX(x), util.rowToY(y))    
        
        # checkpoints part
        cp = self.checkPoints
        if(goalPos == cp[chkPtsSoFar]):
            chkPtsSoFar += 1
